...
function Room(type, config) {
   ...
   /**
     ...
     */
    this.logFilePath = path.join(this.channel.getGameDir(),
                                 'log', (this.name + '.csv'));

    /**
     * ### Room._addNewLine
     *
     * Internal flag if the log should add a new line at the beginning.
     *
     * @api private
     */
    this._newLineLog = fs.existsSync(this.logFilePath);
    ///////////////////////////////////////////////////////////////
}
...
Room.prototype.log = function(arr) {
    ...
    // See if we need to make a new line.
    if (this._newLineLog) this.cacheToSave += "\n";
    else this._newLineLog = true;
    // Prepare log string.
    str = new Date().toISOString();
    str = str.substring(0, str.length - 5);
    // Log: Time,Session,Data.
    this.cacheToSave += str + "," + this.channel.session + "," + arr.join(',');
    if (!this.timeoutSave) {
        that = this;
