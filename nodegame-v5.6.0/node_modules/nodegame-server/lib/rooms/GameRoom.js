 ...
 *   - header: ([ 'id' 'access', 'exit', 'bonus' ]): The name of the columns
 *             in the dump file
...
GameRoom.prototype.computeBonus = function(options) {
    ...
    var winProperty, winDecimals, exchangeRate, missing;
    ...
    if ('undefined' !== typeof options.header) {
        ...
        header = [ 'id', 'access', 'exit', 'bonus' ];
        headerKeys = [ 'id', 'AccessCode', 'ExitCode', winProperty ];
    }
    ...
    if ('undefined' !== typeof options.headerKeys) {
        if (!J.isArray(options.headerKeys) ||
            ...
            throw new Error('GameRoom.computeBonus: options.headerKeys ' +
                            ...
                            'length of options.header. Found: ' +
                            options.headerKeys);
        }
        headerKeys = options.headerKeys;
    }
    else if (!headerKeys) {
        // headerKeys might have been set before.
        headerKeys = header;
    }
    ...
    clients.forEach(function(p) {
        ...
        // Send msg.
        if (options.say) node.say('WIN', p.id, obj);

        // Add id (no need to send it in say, so we do it afterwards.
        obj.id = p.id;
        obj.exit = obj.exit || missing;
        obj.access = accessCode || missing;
        ...
        if (options.dump) {
            if (headerLen == 1) {
                outStr += 'undefined' !== typeof client[headerKeys[0]] ?
                    client[headerKeys[0]] : missing;
            }
            else if (headerLen == 2) {
                outStr +=
                    ('undefined' !== typeof client[headerKeys[0]] ?
                     client[headerKeys[0]] : missing) + ',' +
                    ('undefined' !== typeof client[headerKeys[1]] ?
                     client[headerKeys[1]] : missing);
            }
            else if (headerLen == 3) {
                outStr +=
                    ('undefined' !== typeof client[headerKeys[0]] ?
                    ...
                    ('undefined' !== typeof client[headerKeys[1]] ?
                     client[headerKeys[1]] : missing) + ',' +
                    ('undefined' !== typeof client[headerKeys[2]] ?
                     client[headerKeys[2]] : missing);
            }
            else if (headerLen == 4) {
                outStr +=
                    ('undefined' !== typeof client[headerKeys[0]] ?
                    ...
                    ('undefined' !== typeof client[headerKeys[1]] ?
                     ...
                     ('undefined' !== typeof client[headerKeys[2]] ?
                      client[headerKeys[2]] : missing) + ',' +
                    ('undefined' !== typeof client[headerKeys[3]] ?
                     client[headerKeys[3]] : missing);
            }
            else {
                i = -1;
                for ( ; ++i < headerLen ; ) {
                    outStr += 'undefined' !== typeof client[headerKeys[i]] ?
                        client[headerKeys[i]] : missing;
...
function writeBonusFile(filename, header, content, sysLogger, append) {
    ...
    if (!append) bonusFile.write(header.join(',') + '\n');
