...
function ChannelRegistry(options) {
    if ('object' !== typeof options) {
        throw new TypeError('ChannelRegistry: options must be object. Found: ' +
                            options);
    }
    if ('string' !== typeof options.channelName) {
        throw new TypeError('ChannelRegistry: options.channelName must ' +
                            'be string. Found: ' + options.channelName);
    }
    ...
    this.clients = new PlayerList({
        ...
        I: {
            ...
            connected: function(p) {
                return p.connected ? p.id : undefined;
            },
            ...
            connectedPlayer: function(p) {
                return p.connected && !p.admin ? p.id : undefined;
            },
            sid: function(p) {
 ...
 *   except the `id` field
 * @param {boolean} doUpdate Optional. If TRUE, registry indexes are updated.
 *   Default: TRUE
 *
 ...
 */
ChannelRegistry.prototype.addClient = function(clientId, clientObj, doUpdate) {
    var res;
    ...
    // Add client and update indexes.
    if ('undefined' === typeof doUpdate || doUpdate) {
        res = this.clients.add(clientObj);
    }
    // Add client without updating indexes.
    else {
        res = this.clients.add(clientObj, { indexes: false  });
    }
...
ChannelRegistry.prototype.removeClient = function(client) {
    ...
    if (!this.clients.remove(clientId)) {
        throw new Error('ChannelRegistry.removeClient: error with client: ' +
                        clientId);
    }
};
...
ChannelRegistry.prototype.moveClient = function(client, newRoom) {
    var clientId, roomStack, oldRoom;
...
ChannelRegistry.prototype.checkOut = function(client) {
    this.updateClient(client, { checkout: true });
};
...
/**
 * ### ChannelRegistry.isRemote
 *
 * Returns TRUE if a client is connected via any socket rather than SocketDirect
 *
 * @param {string|object} id The id of the client or a client object
 *
 * @return {boolean} TRUE if the client is connected remotely
 */
ChannelRegistry.prototype.isRemote = function(idOrObj) {
    var client;
    if ('string' === typeof idOrObj) {
        client = this.clients.id.get(idOrObj);
        if (!client) {
            throw new Error('ChannelRegistry.isRemote: unknown client: ' +
                            idOrObj);
        }
    }
    else if ('object' === typeof idOrObj) {
        client = idOrObj;
    }
    else {
        throw new Error('ChannelRegistry.isRemote: idOrObj must be string ' +
                        'or object. Found: ' + idOrObj);
    }
    return client.sid.charAt(0) !== 'D';
};

/**
 * ### ChannelRegistry.isAdmin
 *
 * Returns TRUE if a client is an admin (monitor or logic)
 *
 * @param {string|object} id The id of the client or a client object
 *
 * @return {boolean} TRUE if the client is an admin (monitor or logic)
 */
ChannelRegistry.prototype.isAdmin = function(idOrObj) {
    var client;
    if ('string' === typeof idOrObj) {
        client = this.clients.id.get(idOrObj);
        if (!client) {
            throw new Error('ChannelRegistry.isAdmin: unknown client: ' +
                            idOrObj);
        }
    }
    else if ('object' === typeof idOrObj) {
        ...
        throw new Error('ChannelRegistry.isAdmin: idOrObj must be string ' +
                        'or object. Found: ' + idOrObj);
    }
    return !!client.admin;
};
...
ChannelRegistry.prototype.deregisterClientRoomAliases = function(client, room) {
    ...
    if ('string' !== typeof client) {
        throw new TypeError('ChannelRegistry.deregisterClientRoomAliases: ' +
                            'client must be a string. Found: ' + client);
    }
    if ('string' !== typeof room) {
        throw new TypeError('ChannelRegistry.deregisterClientRoomAliases: ' +
                            'room must be a string. Found: ' + room);
    }
...
ChannelRegistry.prototype.claimId = function(tag) {
    ...
    // TODO: if this changes, update also AdminServer reply to AUTH info.
