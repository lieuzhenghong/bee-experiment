...
function ServerChannel(options, servernode, sio) {
    ...
    /**
     * ### ServerChannel.logClients
     *
     * If TRUE, each room connection/disconnection is logged
     *
     * Note: rooms can overwrite this setting.
     *
     * Default: FALSE
     */
    this.logClients = options.logClients || false;

    /**
     * ### ServerChannel.logClientsExtra
     *
     * A callback adding extra information for every client log
     *
     * Note: rooms can overwrite this setting.
     *
     * Callback takes the full client object and must return an array
     * of strings to be serialized.
     *
     * Default: NULL
     *
     * @see ServerChannel.logClients
     */
    this.logClientsExtra = options.logClientsExtra || null;

    /**
     * ### ServerChannel.logClientsInterval
     *
     * How often in milliseconds clients log are dumped to file system
     *
     * Note: rooms can overwrite this setting.
     *
     * Default: 1000
     ...
     * @see ServerChannel.logClients
     */
    this.logClientsInterval = options.logClientsInterval || 1000;

    /**
     * ### ServerChannel.roomCounterPadChars
     *
     * The number of chars for the roomCounter when saved as a directory on fs
     *
     * If room counter is less than roomCounterPadChars,
     * leading zeros are added.
     *
     * @see padFileNameNumber
     */
    if ('undefined' === typeof options.roomCounterPadChars) {
        this.roomCounterPadChars = 6;
    }
    else {
        this.roomCounterPadChars = options.roomCounterPadChars;
    }

    /**
     * ### ServerChannel.roomCounterSeparator
     *
     * A character to be inserted between 'room' and the room counter.
     *
     * This feature is not implemented now, and reserved for future versions.
     */
    this.roomCounterSeparator = '';
    //     if ('undefined' === typeof options.roomCounterSeparator) {
    //         this.roomCounterSeparator = '';
    //     }
    //     else {
    //         this.roomCounterSeparator = options.roomCounterSeparator;
    //     }
...
ServerChannel.prototype.createGameRoom = function(conf) {
    ...
    if ('undefined' === typeof conf.name) {
        ...
        do {
            conf.name = roomGroup + this.roomCounterSeparator +
                padFileNameNumber(this.autoRoomNo, this.roomCounterPadChars);
            this.autoRoomNo++;
        }
    ...
    // Log Clients
    //////////////
    // Is here the right place? Duplicated in createRoom.
    conf.logClients = 'undefined' === typeof conf.logClients ?
       this.logClients : conf.logClients;
    conf.logClientsExtra = 'undefined' === typeof conf.logClientsExtra ?
       this.logClientsExtra : conf.logClientsExtra;
    conf.logClientsInterval =
       'undefined' === typeof conf.logClientsInterval ?
       this.logClientsInterval : conf.logClientsInterval;
    //////////////////////////////////////////////////////
...
ServerChannel.prototype.createRoom = function(type, settings) {
    ...
    // Log Clients
    //////////////
    // Is here the right place? Duplicated in createGameRoom.
    conf.logClients = 'undefined' === typeof settings.logClients ?
        this.logClients : settings.logClients;
    conf.logClientsExtra = 'undefined' === typeof settings.logClientsExtra ?
        this.logClientsExtra : settings.logClientsExtra;
    conf.logClientsInterval =
        'undefined' === typeof settings.logClientsInterval ?
        this.logClientsInterval : settings.logClientsInterval;

    delete settings.logClients;
    delete settings.logClientsExtra;
    ////////////////////////////////
...
/**
 * ### ServerChannel.getLiveInfo
 *
 * Returns an object containing information about currently connected clients
 *
 * @see ChannelRegistry
 */
ServerChannel.prototype.getLiveInfo = function() {
    return {
        nGameRooms: J.size(this.gameRooms),
        nConnClients: J.size(this.registry.getConnected()),
        nConnAdmins: J.size(this.registry.getConnectedAdmins()),
        nConnPlayers: J.size(this.registry.getConnectedPlayers()),
        nDisconnClients: J.size(this.registry.getDisconnected()),
        nDisconnAdmins: J.size(this.registry.getDisconnectedAdmins()),
        nDisconnPlayers: J.size(this.registry.getDisconnectedPlayers())
    };
};
...
ServerChannel.prototype.connectPhantom = function(options) {
    ...
    phPath = require(path.join(phPath, 'lib', 'phantomjs')).path;
