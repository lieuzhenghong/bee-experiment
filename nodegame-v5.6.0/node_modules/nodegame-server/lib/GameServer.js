...
function GameServer(options, channel) {
     ...
     *
     * If TRUE, clients can reconnect to same game room
     *
     * Clients will be matched with previous game history based on
     * the clientId found in their cookies.
     */
    this.enableReconnections = !!options.enableReconnections;

    /**
     * ### GameServer.antiSpoofing
     *
     * If TRUE, server asks clients to send along their sid in each msg
     */
...
GameServer.prototype.validateGameMsg = function(gameMsg) {
    ...
    if (gameMsg.from.trim() === '') {
        this.sysLogger.log(server + 'Server.validateGameMsg: Received msg ' +
    ...
    if (!this.channel.registry.clients.exist(gameMsg.from)) {
        this.sysLogger.log(server + 'Server.validateGameMsg: Received msg ' +
                           'from unknown client: ' + gameMsg.from, 'error');
        return false;
    }
...
GameServer.prototype.onMessage = function(msg, msgToLog) {
    ...
    this.msgLogger.log(msg, 'in', msgToLog);
 ...
 * @param {string} clientId Optional. Specifies the client id
 *
...
GameServer.prototype.onConnect = function(socketId, socketObj, handshake,
    ...
    if (!handshake.headers || !handshake.headers.cookie) cookies = {};
    else cookies = parseCookies(handshake.headers.cookie);
...
//     }
    this.socketManager.send(this.msg.create({
        ...
        data: {
            ...
            antiSpoofing: this.antiSpoofing &&
                this.socketManager.isRemote(socketId)
        }
    }));
};
...
GameServer.prototype.authorization = function(cb) {
    if ('function' !== typeof cb) {
        throw new TypeError('GameServer.authorization: cb must be ' +
                            'function. Found: ' + cb);
    }
    this.authCb = cb;
};
...
GameServer.prototype.validateRecipient = function(gameMsg) {
    ...
    if ('string' === typeof gameMsg.to) {
        ...
        this.sysLogger.log(server + 'Server.validateRecipient: ' +
                           'gameMsg.to is neither string nor array. Found: ' +
                           gameMsg.to, 'error');
        return false;
    }
    return gameMsg;
